...
# DBTITLE 1,RMD 이상 탐지 분석 템플릿
# MAGIC %md
# MAGIC # RMD (Road Mark Detection) 이상 탐지 분석 템플릿
# MAGIC > 전체 셀을 순서대로 실행하세요. 데이터 준비 셀이 temp view `rmd_events`를 생성한 후, 각 STEP이 이를 참조합니다.
# MAGIC
# MAGIC ### 워크플로우 구조
# MAGIC ```
...
# DBTITLE 1,데이터 준비: rmd_events temp view
# MAGIC %sql
# MAGIC -- 데이터 준비: raw 테이블에서 RMD 이벤트를 파싱하여 temp view 생성
# MAGIC CREATE OR REPLACE TEMP VIEW rmd_events AS
...
# MAGIC     AND e._raw:data.feature::STRING = 'rmd'
# MAGIC )
# MAGIC SELECT
...
# MAGIC        COUNT(DISTINCT user_name) AS user_count,
# MAGIC        COUNT(DISTINCT object_id) AS object_count
# MAGIC FROM rmd_events
...
# MAGIC > position, height 등 JSON 파싱이 필요한 항목. Staging 테이블 안정화 시 참조 교체 가능.
...
# MAGIC WITH rmd_movements AS (
# MAGIC   SELECT session_id, user_name, object_id, object_type, event_time,
# MAGIC          get_json_object(old_val, '$.position[0]') AS old_x,
# MAGIC          get_json_object(old_val, '$.position[1]') AS old_y,
# MAGIC          get_json_object(old_val, '$.position[2]') AS old_z,
# MAGIC          get_json_object(new_val, '$.position[0]') AS new_x,
# MAGIC          get_json_object(new_val, '$.position[1]') AS new_y,
# MAGIC          get_json_object(new_val, '$.position[2]') AS new_z
# MAGIC   FROM rmd_events
# MAGIC   WHERE event_type IN (
# MAGIC       'annotation.point.move', 'annotation.point.move_batch',
# MAGIC       'annotation.polywall.transform'
# MAGIC     )
# MAGIC     AND old_val IS NOT NULL AND new_val IS NOT NULL
# MAGIC ),
# MAGIC distances AS (
# MAGIC   SELECT session_id, user_name, object_id, object_type,
...
# MAGIC GROUP BY severity, object_type

# COMMAND ----------

# DBTITLE 1,[S1-B] STEP 2: 위치 진동
# MAGIC %sql
# MAGIC -- [Stage 1 · Group B] STEP 2: 위치 진동 (reversal 비율 기준, 이동거리 0.05m 이상만 카운트)
# MAGIC -- RMD: polywall + point 혼합, 비율 ≥ 75%일 때 이상으로 판정
# MAGIC WITH ordered AS (
# MAGIC   SELECT session_id, object_id, event_time,
# MAGIC          CAST(get_json_object(new_val, '$.position[0]') AS DOUBLE) AS x,
# MAGIC          CAST(get_json_object(new_val, '$.position[1]') AS DOUBLE) AS y,
# MAGIC          CAST(get_json_object(new_val, '$.position[2]') AS DOUBLE) AS z,
# MAGIC          ROW_NUMBER() OVER (PARTITION BY session_id, object_id ORDER BY event_time) AS rn
# MAGIC   FROM rmd_events
# MAGIC   WHERE event_type IN ('annotation.point.move', 'annotation.point.move_batch')
# MAGIC     AND new_val IS NOT NULL
# MAGIC ),
# MAGIC deltas AS (
# MAGIC   SELECT a.session_id, a.object_id, a.rn,
...
# MAGIC   JOIN ordered b ON a.session_id = b.session_id AND a.object_id = b.object_id AND b.rn = a.rn + 1
# MAGIC ),
# MAGIC significant_deltas AS (
# MAGIC   SELECT *, ROW_NUMBER() OVER (PARTITION BY session_id, object_id ORDER BY rn) AS sig_rn
# MAGIC   FROM deltas
# MAGIC   WHERE move_dist >= 0.05
# MAGIC ),
# MAGIC direction_changes AS (
# MAGIC   SELECT a.session_id, a.object_id, a.sig_rn,
...
# MAGIC   JOIN significant_deltas b ON a.session_id = b.session_id AND a.object_id = b.object_id AND b.sig_rn = a.sig_rn + 1
# MAGIC ),
# MAGIC windowed AS (
# MAGIC   SELECT session_id, object_id, sig_rn,
# MAGIC          SUM(is_reversal) OVER (PARTITION BY session_id, object_id ORDER BY sig_rn ROWS BETWEEN 5 PRECEDING AND CURRENT ROW) AS reversals_in_window,
# MAGIC          COUNT(*) OVER (PARTITION BY session_id, object_id ORDER BY sig_rn ROWS BETWEEN 5 PRECEDING AND CURRENT ROW) AS window_size
...
# DBTITLE 1,[S1-B] STEP 4: 높이 이상
# MAGIC %sql
# MAGIC -- [Stage 1 · Group B] STEP 4: 높이 이상 (height > 0.5m ⚠️ / > 1.0m ❌, 도로면 이탈)
# MAGIC WITH rmd_heights AS (
# MAGIC   SELECT session_id, user_name, object_id, event_time,
# MAGIC          CAST(get_json_object(new_val, '$.height') AS DOUBLE) AS height
# MAGIC   FROM rmd_events
# MAGIC   WHERE event_type IN ('annotation.polywall.create', 'annotation.polywall.transform')
# MAGIC     AND new_val IS NOT NULL
# MAGIC )
# MAGIC SELECT CASE WHEN ABS(height) > 1.0 THEN '>1.0m (critical)'
# MAGIC             WHEN ABS(height) > 0.5 THEN '0.5-1.0m (warning)'
# MAGIC             ELSE 'normal' END AS severity,
# MAGIC        COUNT(*) AS event_count,
# MAGIC        ROUND(AVG(ABS(height)), 4) AS avg_height,
# MAGIC        ROUND(MAX(ABS(height)), 4) AS max_height
# MAGIC FROM rmd_heights
# MAGIC GROUP BY severity
