...
# DBTITLE 1,LD 이상 탐지 분석 템플릿
# MAGIC %md
# MAGIC # LD (Lane Detection) 이상 탐지 분석 템플릿
# MAGIC > 전체 셀을 순서대로 실행하세요. 데이터 준비 셀이 temp view `ld_events`를 생성한 후, 각 STEP이 이를 참조합니다.
# MAGIC
# MAGIC ### 워크플로우 구조
# MAGIC ```
# MAGIC Stage 1 — 수집 데이터 무결성 검증
# MAGIC ├── [Group A] STEP 0 → STEP 3  (Raw 직접, 파싱 불필요)
# MAGIC └── [Group B] STEP 1 → STEP 2  (인라인 파싱, STEP 4 해당 없음)
# MAGIC
...
# DBTITLE 1,데이터 준비: ld_events temp view
# MAGIC %sql
# MAGIC -- 데이터 준비: raw 테이블에서 LD 이벤트를 파싱하여 temp view 생성
# MAGIC CREATE OR REPLACE TEMP VIEW ld_events AS
...
# MAGIC     AND e._raw:data.feature::STRING = 'ld'
# MAGIC )
# MAGIC SELECT
...
# MAGIC > position 등 JSON 파싱이 필요한 항목. Staging 테이블 안정화 시 참조 교체 가능.
# MAGIC >
# MAGIC > **참고**: LD는 STEP 4 (피처별 기하학적 이상) 해당 없음.
...
# MAGIC ),
# MAGIC distances AS (
# MAGIC   SELECT session_id, user_name, object_id,
...
# MAGIC -- LD: 미세 조정이 잦으므로 비율 ≥ 83%일 때만 이상으로 판정
...
# MAGIC        COUNT(*)                            AS `이벤트수`,
# MAGIC        CASE WHEN COUNT(*) >= 10 AND MAX(reversal_ratio) >= 0.83 THEN '심각'
# MAGIC             WHEN MAX(reversal_ratio) >= 0.83 THEN '주의'
# MAGIC             ELSE '정상' END              AS `심각도`
# MAGIC FROM with_ratio
# MAGIC GROUP BY session_id, object_id
# MAGIC HAVING MAX(reversal_ratio) >= 0.83
