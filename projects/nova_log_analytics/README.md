...
Labelit 워크스페이스 커맨드 로그를 기반으로 라벨러 행동 패턴을 분석하는 Databricks 기반 프로젝트입니다.

| 분석 영역 | 상태 | 설명 |
|-----------|------|------|
| 이상 탐지 (Anomaly Detection) | ✅ 구현 완료 | 비정상 편집 패턴 탐지, STEP 0–9, 스코어링 v1.2 |
| KPI 메트릭 설계 및 분석 | 🔜 미전개 | 라벨러 생산성·효율성 지표 설계 및 집계 (`docs/policies/` 선행 자료 참고) |
