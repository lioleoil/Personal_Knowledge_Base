...
│
├── nova_helper/         # Nova Slack 봇 + Confluence 연동 모듈
│   ├── nova_helper.py       → Slack Bolt 앱, 플러그인 로더
│   ├── nova_okr.py          → OKR 관련 명령 핸들러
│   ├── confluence_plugin.py → Confluence 페이지 생성/업데이트 플러그인
│   ├── pm_pipeline.py       → PM 스킬 파이프라인 연동
...
│   ├── doc_gen.py           → Gemini → Confluence 문서 생성 (GUI 모드 지원)
│   ├── scrap_popup.py       → 스크랩 완료 알림 팝업 (좌상단)
│   ├── run_daily_scrap.bat  → Task Scheduler 진입점 (매일 09:00)
│   └── skill_backups/       → PM 스킬 백업 (pm-execution 하위 스킬 원본 보관)
│
└── .status/             # 토큰 추적 및 에이전트 모니터링
    ...
    ├── TOKEN_POLICY.md      → Claude Pro 토큰 사용량 정책 문서
...
| 2026-04-04 | nova_helper/ 모듈 추가 (Slack Bolt + Confluence 연동), pm_ppt_generator.py / pm_skill.py 루트 추가, .scripts/doc_gen.py + skill_backups/ 추가, .status/TOKEN_POLICY.md 추가 |
